...
//Alex Filip
int ready_threads;
void update_ready_threads_counter(void);
...
void thread_start(void) {
	...
	//Alex Filip
	load_avg = fixed_point_convert_int(0);
	ready_threads = 0;
...
void thread_tick(void) {
	...
	if (thread_mlfqs) {
		...
		current->recent_cpu_time = fixed_point_add_int(current->recent_cpu_time, 1);

		if (0 == timer_ticks() % TIMER_FREQ) {
			//Alex Filip
			/* update nr of ready threads*/
			update_ready_threads_counter();

			/* recompute the load_average */
			thread_foreach(recompute_load_avg, NULL);

			/* recompute the recent_cpu_time for all threads*/
			thread_foreach(thread_recompute_recent_cpu, NULL);
		}
               ...
               thread_func *function, void *aux) 
{
	...
	if(thread_mlfqs)
	{
		thread_recompute_priority(t, NULL);
	}
...
thread_set_priority (int new_priority) 
{
    ...
    current->donated = false;
	  if(list_empty(&current->donation_list))
		  ...
		  struct thread *don_thrd = list_entry(list_front(&current->donation_list), struct thread, elem);
...
thread_get_recent_cpu (void) 
{
	//Alex Filip
	struct thread *current = thread_current();
	return current->recent_cpu_time;
}

...
//Alex Filip
void
thread_recompute_recent_cpu(struct thread *t, void *aux UNUSED)
{
	if(t != idle_thread)
	{
		fixed_point_t doubleLoadAvg = fixed_point_mul_int(load_avg, 2);
		fixed_point_t doubleLoadAvg1 = fixed_point_add_int(doubleLoadAvg, 1);

		fixed_point_t recent_cpu = fixed_point_div(doubleLoadAvg, doubleLoadAvg1);
		recent_cpu = fixed_point_mul(recent_cpu, t->recent_cpu_time);
		t->recent_cpu_time = fixed_point_add_int(recent_cpu, t->nice);
	}
}

//Alex Filip
void
recompute_load_avg(struct thread *t, void *aux UNUSED)
{
	if(t!=idle_thread)
	{
		fixed_point_t proc_load_avg = fixed_point_mul_int(load_avg, 59);
		proc_load_avg = fixed_point_div_int(proc_load_avg, 60);

		fixed_point_t proc_ready_threads = fixed_point_div_int(fixed_point_convert_int(ready_threads), 60);

		load_avg = fixed_point_add(proc_load_avg, proc_ready_threads);
	}
}

//Alex Filip
void
thread_recompute_priority(struct thread *t, void *aux UNUSED)
{
	if(t != idle_thread)
	{
		fixed_point_t prioQr = fixed_point_div_int(t->recent_cpu_time, 4);
		int thread_priority = PRI_MAX - fixed_point_to_integer(prioQr);
		thread_priority = thread_priority - (t->nice * 2);
...
//Alex Filip
void
update_ready_threads_counter(void)
{
...
{
	struct thread *cur = thread_current();
	struct lock *lock = cur->blocking_lock;
	int i = 1;

	while(lock != NULL && i<= MAX_DEPTH)
	{
		i++;
		//if there is no lock holder or the thread that is holding the lock has greater priority than current, return
		if(lock->holder == NULL || lock->holder->priority >= cur->priority)
			return;
		else{
			lock->holder->priority = cur->priority; //change priorities
			lock->holder->donated = true;
			cur = lock->holder; 	//update current thread = lock holder
			lock = cur->blocking_lock; 	//update lock
		}
	}
}
